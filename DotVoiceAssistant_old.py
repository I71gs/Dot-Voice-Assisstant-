...
def take_command():
    """Take voice command from microphone and return as text"""
    r = sr.Recognizer()
...
def get_system_info():
    ...
    try:
        ...
        print_error(f"Could not fetch system info: {e}")
...
def show_help():
    ...
    commands = {
        ...
        'play music': 'Play random music from Music folder',
        'system info': 'Display system resources',
        'search': 'Search on Google (say: search [query])',
        'open': 'Open website (say: open [website])',
        'list files': 'List files in current directory',
        'joke': 'Tell a joke',
        'note': 'Take a voice note',
        'open app': 'Open application (say: open [app_name])',
        'help': 'Show this help menu',
        'bye': 'Exit the assistant',
    }
    
    for cmd, desc in commands.items():
        print(f"  {Colors.CYAN}{cmd:<15}{Colors.ENDC} → {desc}")
...
def get_network_info():
    ...
    try:
        ...
        print_error(f"Network info error: {e}")
...
def get_news():
    ...
    try:
        ...
        # Using a free endpoint without key for demo
        url = "https://api.currentsapi.services/v1/latest-news?apikey=test"
        
        print_info("News feature requires API key. Install: pip install newsapi-python")
        speak("News feature requires API setup")
    except Exception as e:
        print_error(f"News error: {e}")
...
def mark_todo_complete(task_index):
    ...
    try:
        ...
        if 0 <= task_index < len(todos):
            ...
            with open(TODO_FILE, 'w') as f:
                json.dump(todos, f, indent=2)
            print_success(f"Task completed: {todos[task_index]['task']}")
        else:
            print_error("Invalid task number")
    except Exception as e:
        print_error(f"Complete TODO error: {e}")

def set_reminder(message, hours=1):
    ...
    try:
        ...
        print_success(f"Reminder set for {reminder_time.strftime('%H:%M')}")
...
def text_to_pdf(text, filename="output.pdf"):
    ...
    try:
        ...
        print_success(f"PDF created: {filename}")
        speak("PDF file created")
    except Exception as e:
        print_error(f"PDF error: {e}")
...
def play_radio(station_url="http://stream.antena3.ro:8000"):
    """Play online radio"""
...
def background_listener_mode():
    """Background listening mode - always active"""
    print_header("BACKGROUND LISTENER ENABLED")
    print_info("Assistant is always listening for commands...")
...
def show_help():
    ...
    print_info("Type 'help' for available commands or speak a command after listening prompt")
        ...
        while True:
            ...
            if 'time' in query:
                ...
                if search_query:
                    ...
                    print_warning("What should I search for?")
            
            elif 'open' in query and 'website' not in query and 'app' in query:
                app = query.replace('open', '').replace('app', '').strip()
                open_application(app)
            
            elif 'open' in query and 'website' not in query and 'app' not in query:
